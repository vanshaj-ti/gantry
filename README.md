...
One-line install (macOS/Linux — clones to `~/gantry`, sets up the venv, adds
`gantry` to PATH, and installs `gh`/`tmux`/`fzf`/`glow` for you):
...
**Other tools used by specific commands.** `install.sh` installs `gh`, `tmux`,
`fzf`, and `glow` for you (see Install above) — `herdr` is the one genuinely
optional tool here, an alternative cockpit integration you opt into yourself:
...
The fastest path — one command scaffolds `gantry.toml`, builds the Docker image
(Claude Code + Codex both installed, isolated from your host), and starts this
project's container ticking `gantry advance --all` on its own:

```bash
cd /path/to/your/repo
gantry setup                     # init + docker build + docker up, one shot
gantry run --title "add health endpoint" --request "add GET /health -> 200"
gantry watch                     # dashboard of all runs, incl. running cost
```

That's it — the container advances the run through every stage on its own,
pausing only at spec/design's human-review gates (unless
`[git].auto_approve_docs = true`, see below). No Docker? Run directly on the
host instead:
...
# doc-stage gates (human) — only if "spec"/"design" are in [stages]:
...
**Stages.** The full pipeline is `spec → design → plan → build → evidence →
review`. Doc stages (`spec`, `design`) produce a markdown artifact
(`product-spec.md`, `architecture-design.md`) and pause at a human-review gate
by default — advance with `gantry approve --stage spec/design`, send back
with `gantry revise`. Set `[git].auto_approve_docs = true` to have Gantry
approve its own spec/design output automatically instead, for a fully
hands-off run. Agent stages (`plan`, `build`, `evidence`) invoke the
configured runner with the stage's prompt. The `review` stage feeds the diff +
artifacts to an independent LLM and parses an `APPROVE` / `REQUEST_CHANGES` /
`ESCALATE` verdict. `stages` in `gantry.toml` defaults to
`["plan", "build", "evidence", "review"]` for existing projects — add `"spec"`
and `"design"` to the front to opt into the full pipeline (see
`gantry/templates/gantry.toml`).

**Runners (pluggable).** `claude-code`, `cursor-cli`, and `codex-cli` ship in v1
— `gantry setup`'s Docker image installs both Claude Code and Codex, so either
can drive any stage with no extra setup. Pick a runner globally via
`[agent] runner = "..."`, or override per-stage via `[models.<stage>] runner =
"..."` — any of the three can drive any stage, including an independent runner
for `[review]` (e.g. build with `claude-code`, review with `codex-cli`, for a
genuinely independent second opinion). `gantry doctor` (and `gantry setup`)
report which runner CLIs are actually available on the current PATH. Add a new
...
**Fully autonomous spec-to-PR (opt-in).** Combine `[git].auto_approve_docs =
true` with `[git].auto_ship = true` (and `stages` including `"spec"` and
`"design"`) for a run that goes from `gantry run --request "..."` straight to
an opened PR with zero human gates at all — spec and design approve
themselves, checks/evidence/review run unattended, and ship fires the moment
review approves. Each flag is independent, so you can keep spec/design
human-gated while still auto-shipping, or vice versa.
...
records cost/token usage. Claude Code and Cursor CLI report `cost_usd`
directly in their JSON output. Codex CLI has no per-token USD field in its own
`--json` stream (ChatGPT/gateway auth isn't billed per-token via the CLI) —
Gantry falls back to [ccusage](https://ccusage.com) (`npm install -g
ccusage`, already bundled in the `gantry setup` Docker image), which computes
real $ cost from the local `~/.codex/sessions` rollout files against
LiteLLM/gateway pricing, matched by the same session/thread id Gantry already
tracks. `gantry watch`'s COST column shows each run's running total; `gantry
cost` shows the repo-wide total and top-N most expensive runs, `gantry cost
--run ID` a per-stage breakdown.
...
| `[git]` | diff base branch, `auto_ship` (ship automatically on review_approved), `auto_merge` (also squash-merge the PR Gantry just opened), `auto_approve_docs` (auto-approve spec/design like a human `gantry approve` would) |
...
gantry docker {build|up|down|status} [--interval S]
                                         run this project inside its own Docker container
                                         (isolated from the host, claude + codex both installed)
